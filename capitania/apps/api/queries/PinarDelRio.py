...
class ContributorsFromPinarQuery(graphene.ObjectType):
    ...
    contributors_with_different_information_pinar_plate = graphene.List(ClienteType)
    contributors_with_different_information_pinar_name = graphene.List(ClienteType)
    contributors_with_equals_information_pinar = graphene.List(ClienteType)
    ...
    def resolve_contributors_with_different_information_pinar_plate(self, info):
        return Cliente.objects.raw('SELECT C.ID,  C.NIT, C.NOMBRE_COMPLETO, C.DPA, TT.MATRICULA, DIR.DIRECCION FROM DIRECCION@INFOGESTI DIR INNER JOIN CLIENTE_DIRECCION@INFOGESTI C_DIR ON DIR.ID = C_DIR.ID_DIRECCION INNER JOIN CLIENTE@INFOGESTI C ON C.ID = C_DIR.ID_CLIENTE INNER JOIN CLIENTE_TT@INFOGESTI TT ON TT.ID_CLIENTE = C.ID INNER JOIN CORE_PINARDELRIO RECA ON C.NIT = RECA.NUMEROIDENTIDAD WHERE C.UNIDAD BETWEEN 2101 AND 2111 AND TT.MATRICULA <> RECA.CHAPANUEVA')
