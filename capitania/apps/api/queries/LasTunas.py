...
class ContributorsFromLasTunasQuery(graphene.ObjectType):
    ...
    def resolve_contributors_with_equals_information_las_tunas(self, info, city_name=graphene.String()):
        if city_name == 'Manati':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3101 order by NUMEROIDENTIDAD')
        if city_name == 'Puerto Padre':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3102 order by NUMEROIDENTIDAD')
        if city_name == 'Jesus Menendez':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3103 order by NUMEROIDENTIDAD')
        if city_name == 'Majibacoa':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3104 order by NUMEROIDENTIDAD')
        if city_name == 'Las Tunas':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3105 order by NUMEROIDENTIDAD')
        if city_name == 'Jobabo':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3106 order by NUMEROIDENTIDAD')
        if city_name == 'Colombia':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3107 order by NUMEROIDENTIDAD')
        if city_name == 'Amancio':
            return LasTunas.objects.raw(
                'select distinct * from CORE_LASTUNAS l INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON l.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT and DPA = 3108 order by NUMEROIDENTIDAD')
