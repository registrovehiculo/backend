...
class ContributorsFromGranmaQuery(graphene.ObjectType):
    ...
    def resolve_contributors_with_different_information_granma_name(self, info, city_name=graphene.String()):
        if city_name == 'Rio Cauto':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3301 and gm.DPA = 3301 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Cauto Cristo':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3302 and gm.DPA = 3302 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Jiguani':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3303 and gm.DPA = 3303 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Bayamo':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3304 and gm.DPA = 3304 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Yara':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3305 and gm.DPA = 3305 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Manzanillo':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3306 and gm.DPA = 3306 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Campechuela':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3307 and gm.DPA = 3307 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Media Luna':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3308 and gm.DPA = 3308 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Niquero':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3309 and gm.DPA = 3309 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Pilon':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3310 and gm.DPA = 3310 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Bartolome Maso':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3311 and gm.DPA = 3311 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Buey Arriba':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3312 and gm.DPA = 3312 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
        if city_name == 'Guisa':
            return Granma.objects.raw(
                "select distinct * from CORE_GRANMA gm inner join CLIENTE@infogesti cl on cl.NIT = gm.NUMEROIDENTIDAD AND cl.UNIDAD = 3313 and gm.DPA = 3313 AND cl.NOMBRE_COMPLETO <> gm.DATOSPERSONA inner join CLIENTE_TT@infogesti tt on cl.ID = tt.ID_CLIENTE")
