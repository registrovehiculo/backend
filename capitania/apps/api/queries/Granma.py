...
class ContributorsFromGranmaQuery(graphene.ObjectType):
    ...
    def resolve_contributors_missing_in_onat_granma(self, info):
        return Granma.objects.raw('select distinct reca.* from CORE_GRANMA reca left outer join cliente@infogesti c on reca.NUMEROIDENTIDAD = c.nit left outer join cliente_tt@infogesti tt on tt.id_cliente =c.id where tt.id_cliente is null')
