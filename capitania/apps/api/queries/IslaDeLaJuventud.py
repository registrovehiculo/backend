...
class ContributorsFromIslaDeLaJuventudQuery(graphene.ObjectType):
    ...
    def resolve_contributors_with_different_information_isla_de_la_juventud_infogesti(self, info, city_name=graphene.String()):
        isla_de_la_juventud = InfogestiContributors.objects.raw("select distinct * from IG_CONTRIBUYENTE_PN@infogesti info INNER JOIN CORE_ISLADELAJUVENTUD i ON i.NUMEROIDENTIDAD =  info.NIT and info.UNIDAD = 4001 and info.ES_PROPIETARIO_TT = '1' WHERE NOMBRE_COMPLETO <> DATOSPERSONA OR NUMEROIDENTIDAD <> NIT order by NIT")
        return isla_de_la_juventud

    # 4 Contribuyentes totalmente coincidentes
    def resolve_contributors_with_equals_information_isla_de_la_juventud(self, info, city_name=graphene.String()):
        isla_de_la_juventud = IslaDeLaJuventud.objects.raw('select distinct * from CORE_ISLADELAJUVENTUD i INNER JOIN IG_CONTRIBUYENTE_PN@infogesti info ON i.NUMEROIDENTIDAD =  info.NIT WHERE NOMBRE_COMPLETO = DATOSPERSONA  and NUMEROIDENTIDAD = NIT')
        return isla_de_la_juventud
