...
class ContributorsFromSantiagoDeCubaQuery(graphene.ObjectType):
    ...
    def resolve_contributors_missing_in_onat_santiago_de_cuba(self, info):
        return SantiagoDeCuba.objects.raw('select distinct reca.* from CORE_SANTIAGODECUBA reca left outer join cliente@infogesti c on reca.NUMEROIDENTIDAD = c.nit left outer join cliente_tt@infogesti tt on tt.id_cliente =c.id where tt.id_cliente is null')
