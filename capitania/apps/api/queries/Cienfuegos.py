...
class ContributorsFromCienfuegosQuery(graphene.ObjectType):
    ...
    def resolve_contributors_missing_in_onat_cienfuegos(self, info):
        return Cienfuegos.objects.raw('SELECT DISTINCT RECA.* from CORE_CIENFUEGOS reca left OUTER JOIN cliente@infogesti C ON C.NIT = RECA.NUMEROIDENTIDAD WHERE C.NIT IS NULL order by reca.NUMEROIDENTIDAD')
